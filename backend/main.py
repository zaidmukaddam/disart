"""Deep learning and backend code for disart.
...
stub = modal.Stub("disart")

stub.clip_image = (
    modal.Image.debian_slim()
    .apt_install(["git"])
    .pip_install(["h5py", "ftfy", "regex", "tqdm", "numpy", "torch"])
...
def run_clip_text(texts: 'list[str]'):
    ...
    """
    text_tokens = clip.tokenize(texts, truncate=True)  # type: ignore
    with torch.no_grad():
...
def run_clip_images(image_urls: 'list[str]'):
    ...
    for i, resp in enumerate(responses):
        if resp.status_code != 200:
            print(
                f"Received status code {resp.status_code} from URL:", image_urls[i]
            )
        ...
        try:
            ...
            missing_indices.append(i)
    images: list[torch.Tensor] = [preprocess(
        img) for img in original_images]  # type: ignore
...
class Artwork:
    ...
    # modern, european/american, or asian/mediterranean
...
def suggestions(text: str, n: int = 50) -> 'list[SearchResult]':
    ...
    return [
        SearchResult(
            score=50 * (1 + scores[i]),
            artwork=data_by_id[embeddings_ids[i]]
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser("disart")
    ...
    if args.sub == "embed-images":
        ...
        for idx in range(0, len(data), chunk_size):
            ...
            chunked_urls.append(
                [row.image_url for row in data[idx: idx + chunk_size]]
            )
        ...
        print(
            f"Finished embedding {len(all_embeddings)} images out of {len(data)}")
